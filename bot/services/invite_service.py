...
class InviteService:
    ...
    async def create_one_time_link(self, user: User, settings: BotSettings) -> InviteLink:
        ...
        tg_link = await self.bot.create_chat_invite_link(
            chat_id=settings.secret_channel_id,
    ...
    async def revoke(self, link: InviteLink, settings: BotSettings, *, status: InviteLinkStatus) -> None:
        try:
            await self.bot.revoke_chat_invite_link(
                chat_id=settings.secret_channel_id, invite_link=link.telegram_link
            )
    ...
    async def mark_used_and_revoke(self, link: InviteLink, settings: BotSettings) -> None:
    ...
    async def expire_stale_links(self, settings: BotSettings) -> int:
        expired = await self.invite_repo.list_expired_active(datetime.now(timezone.utc))
        for link in expired:
            await self.revoke(link, settings, status=InviteLinkStatus.EXPIRED)
        return len(expired)
